...
public class GetBookingsByTitleTestC {
    ...
    @Test
    @DisplayName("getBookingByTitle with booking present, should return status 200 and booking")
    public void getBookingByTitleOk() throws Exception {
        mockMvc.perform(get("/users/getBookingByTitle/" + event.getTitle())
                .contentType(MediaType.APPLICATION_JSON)
                .header("Authorization", "bearer " + token)
                .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.eventName").value(event.getTitle()));
    }

    @Test
    @DisplayName("getBookingByTitle with no booking present, should return status 404 and message")
    public void getBookingByTitleException() throws Exception {
        booking_repo.deleteAll();
        mockMvc.perform(get("/users/getBookingByTitle/" + event.getTitle())
                        .contentType(MediaType.APPLICATION_JSON)
                        .header("Authorization", "bearer " + token)
